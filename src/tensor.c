...
void tensor_fill(Tensor2D *t, float value) {
    for (int i = 0; i < t->rows * t->cols; i++) {
        t->data[i] = value;
    }
}

void tensor_fill_random(Tensor2D *t, unsigned int seed){
    srand(seed);
    for (int i = 0; i < t->rows * t->cols; i++) {
        t->data[i] = (float)rand() / (float)RAND_MAX; // Random float between 0 and 1
    }
}
...
bool tensor_compare(Tensor2D *a, Tensor2D *b, float epsilon) {
    if (a->rows != b->rows || a->cols != b->cols) {
        return false; // Different dimensions
    }
    for (int i = 0; i < a->rows * a->cols; i++) {
        if (fabs(a->data[i] - b->data[i]) > epsilon) {
            return false; // Values differ more than epsilon
        }
    }
    return true; // Tensors are approximately equal
}
...
void print_tensor(Tensor2D t) {
    for (int r = 0; r < t.rows; r++) {
        for (int c = 0; c < t.cols; c++) {
            printf("%f ", tensor_get_value(&t, r, c));
        }
        printf("\n");
    }
}